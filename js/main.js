...
NCConverter.init = function() {
  console.log("NC Converter initializing...");
  ...
  const coreInitializationPromise = new Promise((resolve) => {
    ...
    const initNextCoreModule = (index) => {
      if (index >= this.config.coreModules.length) {
        console.log(`Core modules initialization complete (${coreModulesInitialized}/${this.config.coreModules.length} succeeded)`);
        resolve();
        return;
      }
  ...
  coreInitializationPromise.then(() => {
    console.log("Starting feature modules initialization");
    ...
    Promise.all(featureInitPromises).then(results => {
      ...
      console.log(`Feature modules initialization complete (${successful}/${this.config.featureModules.length} succeeded)`);
