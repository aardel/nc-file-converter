...
NCConverter.Search = {
  ...
  performSearch: function() {
    const { searchInput, convertedPreview, searchCaseSensitive } = this.elements;
    
    if (!searchInput || !convertedPreview) {
  ...
  getPreviewTextContent: function() {
    ...
    tempDiv.innerHTML = convertedPreview.innerHTML;
  ...
  searchInMainThread: function() {
    const { searchInput, convertedPreview, searchCaseSensitive, searchResultsInfo } = this.elements;
  ...
  highlightMatches: function() {
    const { convertedPreview } = this.elements;
    if (!convertedPreview || this.searchMatches.length === 0) return;
  ...
  findGlobalMatchIndex: function(localMatch) {
    for (let i = 0; i < this.searchMatches.length; i++) {
      ...
      if (globalMatch.text === localMatch.text) {
        // This is an approximation - we assume matches with same text are the same
        // A more accurate approach would require positional tracking
        return i;
      }
    }
    return -1;
  ...
  updateCurrentMatch: function() {
    const { convertedPreview } = this.elements;
    if (!convertedPreview || this.currentMatchIndex < 0 || 
        this.currentMatchIndex >= this.searchMatches.length) return;
    
    // Find the match element by data-match-index
  ...
  clearSearchHighlights: function() {
    const { convertedPreview, prevMatchBtn, nextMatchBtn, searchResultsInfo } = this.elements;
    if (!convertedPreview) return;
  ...
  escapeRegExp: function(string) {
    return string.replace(/[.*+?^${}()|[\]\\]/g, '\\      // Add highlighted match
